...
int rand(void) {
  ...
  return ((unsigned int)next / SHIFT) % MODULUS;
}
...
wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      if(p->state == ZOMBIE){
        // Found one.
        //cprintf("Pid %d - start_time %d - end_time %d - total_time %d - predicted_burst %d - actual burst %d\n", p->pid, p->start_time, p->end_time, p->total_time, p->predicted_burst, p->burst_time);
        pid = p->pid;
        kfree(p->kstack);
        p->kstack = 0;
        freevm(p->pgdir);
...
/*void set_lottery_tickets(int tickets){
	struct proc *p = myproc();
	p->tickets = tickets;
	return;
}*/
...
void set_sched_priority_for_pid(int pid, int priority){
	struct proc *p;
	acquire(&ptable.lock);
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
...
cps()
{
  ...
  acquire(&ptable.lock);
  cprintf("name \t pid \t state \t     start time   end time \t total time \t job length \t burst time \n");
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == SLEEPING)
      cprintf("%s \t %d \t SLEEPING \t %d \t %d \t \t %d \t \t %d \t %d \t %d\n ", p->name, p->pid, p->start_time, p->end_time, p->total_time, p->predicted_burst, p->burst_time, p->priority);
    else if(p->state == RUNNING)
      cprintf("%s \t %d \t RUNNING \t %d \t %d \t \t %d \t \t %d \t \t %d \t %d\n ", p->name, p->pid, p->start_time, p->end_time, p->total_time, p->predicted_burst, p->burst_time, p->priority);
    else if(p->state == RUNNABLE)
      cprintf("%s \t %d \t RUNNABLE \t %d \t %d \t \t %d \t \t %d \t \t%d \t %d\n ", p->name,p->pid,p->start_time, p->end_time, p->total_time, p->predicted_burst, p->burst_time, p->priority);
    else if(p->state == ZOMBIE)
      cprintf("%s \t %d \t ZOMBIE \t %d \t %d \t \t %d \t \t %d \t \t %d \t%d\n ", p->name,p->pid,p->start_time, p->end_time, p->total_time, p->predicted_burst, p->burst_time, p->priority);
  }
  release(&ptable.lock);
  return 22;
}*/
